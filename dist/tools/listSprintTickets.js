...
export async function listSprintTicketsHandler(jira, customFieldsMap, args) {
    ...
    if (includeOpen)
        sprintFunctions.push("openSprints()");
    if (includeFuture)
        sprintFunctions.push("futureSprints()");
    ...
    for (const issue of issues.issues || []) {
        const sprints = (issue.fields[sprintFieldId] || []);
    ...
    const sections = sortedSprints.map(({ sprint, issues }) => {
        ...
        const lines = issues.map((issue) => `  ${issue.key}: ${issue.fields.summary || "No summary"} (${issue.fields.status?.name || "No status"}) [Assignee: ${issue.fields.assignee?.displayName || "Unassigned"}]`);
