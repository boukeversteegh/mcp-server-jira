 ...
 */
export function detectMove(fields) {
    const keys = Object.keys(fields);
    const typeKey = keys.find((k) => TYPE_KEYS.includes(k));
    if (!typeKey)
        return null;
    const otherKeys = keys.filter((k) => k !== typeKey && !PARENT_KEYS.includes(k));
    if (otherKeys.length > 0) {
        return {
            error: `Changing the issue type cannot be combined with other field updates. ` +
 ...
 */
export async function moveIssuesCore(jira, issueKeys, request) {
    ...
    for (const issueKey of issueKeys) {
        try {
            ...
            failLines.push(`${issueKey}: ${formatJiraError("could not read issue", e)}`);
        }
    }
    ...
    for (const snap of snapshots) {
        const available = await getIssueTypes(jira, typesByProject, snap.projectKey);
        const target = available.find((it) => it.name?.toLowerCase() === issueType.toLowerCase());
        if (!target) {
            failLines.push(`${snap.key}: issue type "${issueType}" is not available in project ${snap.projectKey}. ` +
        ...
        if (target.id === snap.currentTypeId && parentUnchanged) {
            unchanged.push(`${snap.key}: already of type ${snap.currentTypeName} (unchanged)`);
            continue;
        }
        const newParent = parentKey ?? (target.subtask ? snap.parentKey : undefined);
        if (target.subtask && !newParent) {
            failLines.push(`${snap.key}: "${target.name}" is a sub-task type, so a parent is required. ` +
    ...
    for (const group of groups.values()) {
        try {
            const submitted = await jira.issueBulkOperations.submitBulkMove({
            ...
            for (const key of group.issueKeys) {
                failLines.push(`${key}: ${formatJiraError(`move to ${group.targetTypeName} failed`, e)}`);
            }
        }
    }
    const sections = [];
...
async function pollBulkMove(jira, taskId) {
    for (const delay of POLL_DELAYS_MS) {
        const progress = await jira.issueBulkOperations.getBulkOperationProgress({
            taskId,
        });
...
function reportGroupOutcome(group, progress, idToKey, successLines, failLines) {
    if (!progress?.status) {
        failLines.push(`${group.issueKeys.join(", ")}: move to ${group.targetTypeName} — no status returned by Jira.`);
        return;
    }
    if (progress.status === "COMPLETE") {
        const failedMap = progress.failedAccessibleIssues &&
        ...
        for (const id of failedIds) {
            ...
            const reason = Array.isArray(failedMap[id])
                ? failedMap[id].join("; ")
                : String(failedMap[id]);
            failLines.push(`${key}: move to ${group.targetTypeName} failed: ${reason}`);
        }
        return;
    }
    if (TERMINAL_STATUSES.includes(progress.status)) {
        failLines.push(`${group.issueKeys.join(", ")}: move to ${group.targetTypeName} ${progress.status.toLowerCase()} (task ${progress.taskId}).`);
        return;
    }
    failLines.push(`${group.issueKeys.join(", ")}: move to ${group.targetTypeName} still ${progress.status.toLowerCase()} — ` +
