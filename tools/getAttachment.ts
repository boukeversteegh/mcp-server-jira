...
export const getAttachmentDefinition = {
  name: "get-attachment",
  description:
    "List and download attachments from a Jira issue. Without attachmentId, lists all attachments. With attachmentId, downloads the attachment — text-based files (markdown, txt, json, etc.) are returned directly, binary files are saved to the specified directory.",
  inputSchema: {
    type: "object",
    properties: {
      issueKey: {
      ...
      attachmentId: {
        type: "string",
        description:
          "The attachment ID to download. If omitted, lists all attachments on the issue.",
      },
      saveTo: {
        type: "string",
        description:
          "Directory path to save the file to. Required for binary files. For text files, if omitted the content is returned directly.",
      },
    },
    required: ["issueKey"],
  },
...
export async function getAttachmentHandler(
  jira: Version3Client,
  args: { issueKey: string; attachmentId?: string; saveTo?: string },
): Promise<McpResponse> {
...
async function downloadAttachment(
  ...
  if (saveTo) {
    ...
    return respond(
      `Saved to: ${filePath} (${formatFileSize(buffer.length)})`,
    );
  }
  ...
  return respond(
    `Attachment "${filename}" is a binary file (${mimeType}, ${formatFileSize(meta.size || 0)}). Use the saveTo parameter to specify a directory to save it to.`,
  );
}
